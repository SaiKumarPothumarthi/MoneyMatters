...
const TransactionItem = (props) => {
  const { id, transactionName, type, category, amount, date, reload, isAdmin } = props;
  const icon =
    type === "credit" ? (
    ...
    <tr className={styles.transactionItem}>
      ...
      <td>
        <div className={styles.buttonsContainer}>
          {!isAdmin && <UpdateTransactionBtn {...props} /> }
          {!isAdmin && <DeleteTransactionButton id={id} reload={reload} isAdmin={isAdmin} />}
        </div>
      </td>
    </tr>
  );
